...
export const getValues = (features, valueProperty, fundingType) => {
  ...
  return values.filter((val) => {
    return val > 0
  });//return array with values (zero values removed)
};
...
export const getStyledGeoJson = (geojson, layerSettings, classProviderInstance) => {
  ...
  const newFeatures = features.map((feature) => {
    ...
    Object.assign(newFeature.properties, {
      ...
      value,
      label
    });//Assign extra data to feature properties
    return newFeature;
  });
