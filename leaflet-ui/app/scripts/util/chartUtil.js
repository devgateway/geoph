        ...
        if (index < itemsToShow){
          if (measureType=='projectCount'){
            if (item.projectCount && parseInt(item.projectCount)>0){
              let label = ellipseString(capitalizeString(item[dimension]), (chartContainerRef? chartContainerRef.offsetWidth/15 : 33)) + (chartType=='pie'?"    ":"")/*workaround for dropped text on print*/;
              labels.push(label);
