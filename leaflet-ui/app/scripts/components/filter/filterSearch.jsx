      ...
      <div className="inner-filter-search">
        <div className="input-group">
          <OverlayTrigger delayShow={1000} placement="top"
                          overlay={(<Tooltip id="help.filters.search">{translate('help.filters.search')}</Tooltip>)}>
            <input ref="keyword" type="text" className="form-control"
                   placeholder={translate('filters.buttons.search') + "..."} onKeyUp={this.onKeyUp.bind(this)}/>
          </OverlayTrigger>
          <span className="input-group-btn">
			        <OverlayTrigger delayShow={1000} placement="top"
                              overlay={(<Tooltip id="help.filters.go">{translate('help.filters.go')}</Tooltip>)}>
						<button className="btn btn-success" type="button"
                    onClick={this.triggerSearch.bind(this)}>{translate('filters.buttons.go')}!</button>
					</OverlayTrigger>
			      </span>
        </div>
